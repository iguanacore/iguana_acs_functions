        ...
        public class UpdateBaseMentalStateSlider
        {
            static void Postfix(Npc npc, UI_Panel_ThingInfo ___Panel)
            {
                try
                {
                    ...
                    {
                        var baseMentalStateBar  = initializeUIProgressBar( ___Panel );
                        currentMentalStateSlider.AddChild( baseMentalStateBar );
                    }
                    if ( baseMentalState < 0 )
                    {
                        currentMentalStateSlider.GetChild( "baseMentalStateBar" ).width = 0;
                    }
                    else
                    {
                        currentMentalStateSlider.GetChild( "baseMentalStateBar" ).width = Math.Min( 1, baseMentalState / 100 ) * currentMentalStateSlider.width;
                    }
                }
                catch ( Exception e )
                {
